...
class CRFEntityExtractor(EntityExtractor):
    ...
    defaults = {
        ...
        "features": [
            ...
            "suffix2", "upper", "title", "digit", "pattern"], # TODO add pos
            ["low", "title", "upper"]
        ],
    ...
    def extract_entities(self, message):
        ...
        if self.ent_tagger is not None:
            ...
            ents = self.ent_tagger.predict_marginals_single(features) # 预测各个标签的概率
    ...
    def _sentence_to_features(self, sentence): # 提取句子的特征，特征设定在default中
        ...
        for word_idx in range(len(sentence)):
            ...
            prefixes = [str(i) for i in feature_range] # prefixes: [-1, 0, 1]
            word_features = {}

            for f_i in feature_range:
                if word_idx + f_i >= len(sentence): # 句子的EOS
                    word_features['EOS'] = True
                    # End Of Sentence
                elif word_idx + f_i < 0: # 句子的BOS
